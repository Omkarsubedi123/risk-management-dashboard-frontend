...
const RiskEdit = () => {
  ...
  const impact = Number(form.impact) || 0;
  ...
  const lossAmount = (estimatedCost * meta.percent) / 100;
  ...
  const hydrateForm = (risk) => {
    setForm({
      ...
      estimated_cost: risk.estimated_cost || 0,
  ...
  if (loading) return <div className="re-loading">Loading risk...</div>;
      ...
      <form className={`re-card ${meta.cls}`} onSubmit={handleSubmit}>
        ...
        <div className="re-grid">
          ...
          <div>
            ...
            <input type="number" min="1" max="5" name="probability" value={form.probability} onChange={handleChange} />
          </div>

          <div>
            <label>Impact (1–5)</label>
            <input type="number" min="1" max="5" name="impact" value={form.impact} onChange={handleChange} />
          </div>
        ...
        <div className="re-calc">
          <div><label>Risk Score</label><p>{riskScore}</p></div>
          <div><label>Risk Level</label><p>{meta.level}</p></div>
          <div><label>Loss %</label><p>{meta.percent}%</p></div>
          <div>
            <label>Loss Amount</label>
            <p className="re-loss">₹{lossAmount.toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="re-actions">
          ...
          <button type="button" className="btn btn-outline" onClick={() => navigate(`/risks/${id}`)}>
            Cancel
          </button>
        </div>
      </form>
